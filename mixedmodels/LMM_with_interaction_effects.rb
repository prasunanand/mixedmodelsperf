...
df_num_x_num = Daru::DataFrame.from_csv '../data2/numeric_x_numeric_interaction.csv', headers: true

puts "======================LMM.from_formula======================"
puts Benchmark.measure{LMM.from_formula(formula: "y ~ a + b + a:b + (0 + a:b | gr)", data: df_num_x_num)}
puts "============================================================"
...
# we set headers: true, because mixed_models expects that all variable names in the data frame
# are ruby Symbols (not Strings, as they would be otherwise)
df_num_x_cat = Daru::DataFrame.from_csv '../data2/numeric_x_categorical_interaction.csv', headers: true

puts "===============LMM.from_formula============================="
puts Benchmark.measure{ LMM.from_formula(formula: "y ~ num + cat + num:cat + (0 + num:cat | gr)", data: df_num_x_cat) }
puts "============================================================"
